...
export class DebateGenerator {
  ...
  private async generatePersonaResponse(
    ...
    // Generate content
    const grokResponse = useTwitterSearch
      ? await this.grokClient.generateWithSearch({
          prompt,
      ...
      : await this.grokClient.generate({
          prompt,
          systemPrompt: persona.systemPrompt,
          model,
          temperature,
    ...
    return {
      ...
      timestamp: new Date().toISOString(),
      twitterIntegrated: useTwitterSearch,
      biasLevel,
      voiceEnhanced
    };
  }
...
TWITTER/X INTEGRATION FOR ${persona.characterName.toUpperCase()}:
- Reference current discourse from your ideological sphere: ${influences}
- Cite trending discussions from trusted sources: ${sources}
- Include viral moments or hashtags relevant to your perspective
- Counter opposing viewpoints with community-noted facts
- Reference threads and conversations that support your position
- Use social media as evidence of grassroots/expert energy and organizing`;
  }
